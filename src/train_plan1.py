...
# --- 1. CONFIGURATION (PLAN 1: INCREASED WIDTH) ---
HIDDEN_SIZE = 64        # Increased capacity (8 -> 64)
EMBED_DIM = 64          # Increased capacity (8 -> 64)
NUM_CLASSES = 5
NUM_EPOCHS = 40         # Enough time to learn
BATCH_SIZE = 16
LEARNING_RATE = 0.001
DROPOUT_RATE = 0.3      # NEW: Dropout against overfitting

# Path Setup: Find the zip in the parent directory (Project Root)
...
def load_data_from_zip(project_root):
    ...
    # Find any .zip file in the project root
    ...
    with zipfile.ZipFile(target_zip, 'r') as z:
        for filename in z.namelist():
            ...
            # 2. Skip MacOS hidden files
            if "__MACOSX" in filename:
                continue

            # 3. IGNORE 'consensus' FOLDER inside the zip
            if "consensus/" in filename:
                continue

            try:
                # Read file from zip memory
...
class LegalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        item = self.data[idx]
        tokens = item['text']
        label = item['label'] - 1 # 1-5 -> 0-4 conversion
...
# --- 4. MODEL ARCHITECTURE (PLAN 1) ---
class LegalModelPlan1(nn.Module):
    def __init__(self, vocab_size, embed_dim, hidden_size, num_classes, dropout_rate):
        super(LegalModelPlan1, self).__init__()
        
        # Larger Embedding
        self.embedding = nn.Embedding(vocab_size, embed_dim, padding_idx=0)
        
        # Larger LSTM (64 neurons)
    ...
    def forward(self, x, lengths):
        ...
        # Pack padded sequence for correct LSTM operation
...
# --- 5. TRAINING LOOP ---
model = LegalModelPlan1(vocab_size, EMBED_DIM, HIDDEN_SIZE, NUM_CLASSES, DROPOUT_RATE)
...
print(f"\n--- Plan 1 Started: Increased Width (Hidden={HIDDEN_SIZE}, Dropout={DROPOUT_RATE}) ---")
...
plt.legend()
plt.tight_layout()
plt.savefig('learning_curve_plan1.png')
print("Learning curve saved: learning_curve_plan1.png")
...
metrics_file = "plan1_metrics.txt"
with open(metrics_file, "w") as f:
    f.write("Plan 1 Model Evaluation Metrics\n")
    f.write("===============================\n")
...
plt.ylabel('True')
plt.title('Confusion Matrix - Plan 1')
plt.savefig('confusion_matrix_plan1.png')
print("Confusion Matrix saved: confusion_matrix_plan1.png")